...
namespace gpu_lab {
  ...
  namespace detail {
    ...
    concept ViewCastCompatible =
      (!std::is_const_v<From> || std::is_const_v<To>) // cannot cast away const
      && (sizeof(From) % sizeof(To) == 0)             // sizeof(To) must divide sizeof(From)
      && (alignof(From) >= alignof(To));              // target type has stricter alignment requirements      
  } // namespace detail
