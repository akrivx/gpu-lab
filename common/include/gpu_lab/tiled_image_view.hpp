...
namespace gpu_lab {
  namespace detail {
    ...
    struct TileAccessor {
      ...
      __host__ __device__ constexpr TileAccessor() noexcept = default;

      __host__ __device__ constexpr TileAccessor(const cuda::std::array<std::size_t, 2>& strides,
                                                 const TileExtents& tile_extents) noexcept
          : strides_{strides}
          , tile_extents_{tile_extents} {}

      __host__ __device__ constexpr reference access(data_handle_type p,
      ...
      __host__ __device__ constexpr data_handle_type offset(data_handle_type p,
                                                            index_type i) const noexcept {
        return p + i;
      }
    };

    template <std::size_t R, std::size_t H, std::size_t W>
    __host__ __device__ constexpr std::size_t get_extent(const ImageViewExtents<H, W>& extents) {
      static_assert(R < 2);
    ...
    DynamicImageViewExtents tile_grid_extents(DynamicImageViewExtents image,
  ...
  template <PitchedElement T, MemoryLocation Loc, typename TileExtents = DynamicImageViewExtents>
  using TiledImageView =
      cuda::std::mdspan<ImageView<T, Loc, TileExtents>, // element_type = tile view
                        DynamicImageViewExtents, // grid extents (currently dynamic)
                        cuda::std::layout_stride,
                        detail::TileAccessor<T, Loc, TileExtents>>;

  template <MemoryLocation Loc, std::size_t TileH, std::size_t TileW, PitchedElement T>
  __host__ __device__ auto
  tiled_image_view(T* data,
    ...
    cuda::std::array<std::size_t, 2> tile_grid_strides{image_strides[0] * tile_h,
                                                       image_strides[1] * tile_w};

    auto mapping = cuda::std::layout_stride::mapping(tile_grid_extents(image_extents, tile_extents),
                                                     tile_grid_strides);
  ...
  tiled_image_view(T* data,
                   ...
                   const DynamicImageViewExtents& image_extents,
                   const ImageViewExtents<TileH, TileW>& tile_extents = {}) {
    return tiled_image_view<Loc>(data, {image_pitch, 1}, image_extents, tile_extents);
  }
  ...
  tiled_image_view(T* data,
                   ...
                   const ImageViewExtents<TileH, TileW>& tile_extents = {}) {
    return tiled_image_view<Loc>(data, {image_pitch, 1}, {image_height, image_width}, tile_extents);
  }

  template <std::size_t TileH, std::size_t TileW, PitchedElement T, MemoryLocation Loc>
  __host__ __device__ auto
  tiled_image_view(ImageView<T, Loc> v, const ImageViewExtents<TileH, TileW>& tile_extents = {}) {
    return tiled_image_view<Loc>(&v(0, 0), {v.stride(0), v.stride(1)}, v.extents(), tile_extents);
  }

  template <PitchedElement T, MemoryLocation Loc>
  __host__ __device__ auto
  tiled_image_view(ImageView<T, Loc> v, std::size_t tile_height, std::size_t tile_width) {
    return tiled_image_view(v, DynamicImageViewExtents{tile_height, tile_width});
  }
